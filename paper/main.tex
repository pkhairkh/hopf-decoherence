...
\item \textbf{$A_2$}: \textbf{Verified (Theorem~\ref{thm:sl3})}. $\dim \HH^2 = 8$.
\item \textbf{$A_3$}: \textbf{Open}. Structural prediction $\dim \operatorname{im} \delta = \binom{4}{2} = 6$ is \emph{refuted} (computed $= 3$). Full count $\dim \HH^2 = 18$ is in doubt: alternative count $15$ is consistent.
\item \textbf{$B_2, G_2$}: structural only.
\end{itemize}
