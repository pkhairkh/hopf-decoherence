...
A Python implementation of the coproduct-rank-deficiency programme for
``u_q(sl_n)`` at roots of unity and of the BCGP non-semisimple TQFT
construction for the BTZ black-hole entropy log correction.

Key results
------------
  * ``D_2(ell) = (ell^3 - ell) / 6`` (sl_2 coproduct rank deficiency).
  * ``ker(Delta)`` is not a Hopf ideal for ``n >= 3``.
  * Verlinde fusion identity: total fusion multiplicity equals ``D_2(ell)``.
  * BCGP full thermal trace: log correction ``-3/2`` (matches gravity).
  * BCGP modified trace: log correction ``-2`` (categorical tool only).
  * Deficiency-log connection: ``D_2(r) ~ r^3/6`` implies
    ``D^2 ~ r^3`` and ``Z_BTZ^full ~ r^{-3/2}``.
  * The ``+1/2`` shift is supplied by the radical states of the
    projective indecomposable modules, which generate the
    ``r^{1/2}`` factor in ``Z_cont``.
  * Euclidean wormhole (double solid torus): log correction ``-5/2``.
  * Multi-boundary generalization: log correction ``-(2n+1)/2`` for
    ``n``-boundary geometries.
...
__email__ = "pkhairkh@users.noreply.github.com"
__license__ = "MIT"
