/*
 * verify_signs.c — Systematically test all sign combinations for ∂ᶜf and ∂ᶜg
 * to find the one that makes d2∘d1=0.
 *
...
/* Sign parameters: s_cf[3] for ∂ᶜf, s_cg[3] for ∂ᶜg */
static int s_cf[3], s_cg[3];
...
static void d2_forward(const dcomplex *v,int nt){
    ...
    for(int32_t i=0;i<g_n_c2;i++){
        ...
        if(g_c2_type[i]==0){
            ...
            /* ∂ᶜf with signs s_cf[0], s_cf[1], s_cf[2] */
            for(int e=0;e<n_db_by_k[af];e++){int idx=db_by_k[af][e];
                c3_accum(hkey(1,delta_bar[idx].c,bf,delta_bar[idx].j,lf),(scomplex)(s_cf[0]*db_val(idx)*vi));}
            for(int e=0;e<n_db_by_c[lf];e++){int idx=db_by_c[lf][e];
                c3_accum(hkey(1,af,bf,delta_bar[idx].j,delta_bar[idx].k),(scomplex)(s_cf[1]*db_val(idx)*vi));}
            for(int e=0;e<n_db_by_j[bf];e++){int idx=db_by_j[bf][e];
                c3_accum(hkey(1,af,delta_bar[idx].c,lf,delta_bar[idx].k),(scomplex)(s_cf[2]*db_val(idx)*vi));}
        } else {
            ...
            /* ∂ᶜg with signs s_cg[0], s_cg[1], s_cg[2] */
            for(int e=0;e<n_db_by_k[cf];e++){int idx=db_by_k[cf][e];
                c3_accum(hkey(2,delta_bar[idx].c,delta_bar[idx].j,jf,kf),(scomplex)(s_cg[0]*db_val(idx)*vi));}
            for(int e=0;e<n_db_by_c[jf];e++){int idx=db_by_c[jf][e];
                c3_accum(hkey(2,cf,delta_bar[idx].j,delta_bar[idx].k,kf),(scomplex)(s_cg[1]*db_val(idx)*vi));}
            for(int e=0;e<n_db_by_j[cf];e++){int idx=db_by_j[cf][e];
                c3_accum(hkey(2,delta_bar[idx].c,jf,kf,delta_bar[idx].k),(scomplex)(s_cg[2]*db_val(idx)*vi));}
        }
    }
}
...
int main(int argc,char*argv[]){
    ...
    printf("=== Sign search for d2∘d1=0 ===\nThreads=%d\n\n",nt);fflush(stdout);
    ...
    printf("d1: %lld nnz\n\n",(long long)d1->nnz);fflush(stdout);
    ...
    /* Build ONE random h and compute ||d1(h)|| once */
    srand(12345);
    ...
    /* Try all 64 sign combinations (2^3 for ∂ᶜf × 2^3 for ∂ᶜg) */
    printf("Testing all 64 sign combinations:\n");
    printf("  s_cf[0] s_cf[1] s_cf[2]  s_cg[0] s_cg[1] s_cg[2]  ratio\n");
    printf("  --------------------------------------------------------\n");fflush(stdout);

    double best_ratio=1e30;
    int best_cf[3],best_cg[3];
    for(int cf=0;cf<8;cf++){
        s_cf[0]=(cf&1)?1:-1; s_cf[1]=(cf&2)?1:-1; s_cf[2]=(cf&4)?1:-1;
        for(int cg=0;cg<8;cg++){
            s_cg[0]=(cg&1)?1:-1; s_cg[1]=(cg&2)?1:-1; s_cg[2]=(cg&4)?1:-1;
            ...
            printf("  %+d  %+d  %+d   %+d  %+d  %+d   %.4e",
                   s_cf[0],s_cf[1],s_cf[2],s_cg[0],s_cg[1],s_cg[2],ratio);
            if(ratio<best_ratio){
                best_ratio=ratio;
                memcpy(best_cf,s_cf,sizeof(s_cf));
                memcpy(best_cg,s_cg,sizeof(s_cg));
                printf(" ***");
            }
            printf("\n");fflush(stdout);
        }
    ...
    printf("\n=== BEST ===\n");
    printf("  s_cf = %+d %+d %+d, s_cg = %+d %+d %+d\n",
           best_cf[0],best_cf[1],best_cf[2],best_cg[0],best_cg[1],best_cg[2]);
    printf("  ratio = %.4e\n",best_ratio);
    printf("  %s\n",best_ratio<1e-8?"PASS ✓":"FAIL ✗");fflush(stdout);

    free(hv);free(vv);free(g_c3);
